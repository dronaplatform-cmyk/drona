...
const ScheduleDemoHeader = ({ heading, description, avatars }: Header) => {
  return (
...
const ScheduleDemoTestimonials = ({ testimonials }: TestimonialsProps) => {
...
const ScheduleDemoFooter = ({ heading, logos }: Footer) => {
  return (
    <div className="flex w-full max-w-6xl flex-col items-center gap-14 text-center">
      ...
      <div className="flex w-full flex-wrap items-center justify-center gap-10 md:grid md:grid-cols-5">
        {logos.map((logo, index) => {
          return (
            <img
              key={`ScheduleDemo-footer-logo-${index}`}
              src={logo}
              alt={`logo ${index + 1}`}
              className={cn(
